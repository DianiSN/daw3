...
//GET alumnos
$app->get('/display/articulos/:userId', function ($userId) {
    
    
        $con=mysqli_connect("localhost","root","","PD3");
        // Check connection
        ...
        $query = "SELECT `Articulo`.`nombre`, `Articulo`.`estadoArticulo`,`Articulo`.`descripcion`, `Articulo`.`calle`, `Estados`.`nombre` AS Estado, `Articulo`.`cuidad`, `Articulo`.`codigoPostal`,`Articulo`.`vendido`  
                                FROM Articulo, Estado
                                WHERE `Articulo`.`state` =  `Estados`.`estadoId`
                                AND `Articulo`.`usuarioId`=".$userId." ORDER BY `Articulo`.`vendido` ASC";
        ...
        while($row = mysqli_fetch_array($result)) {
          ...
          echo "<td>" . $row['calle'] . ",".$row['cuidad']. ",". $row['Estado'].",".$row['codigoPostal']."</td>";
          if($row['vendido']==0){
...
//no funciona porque no me regresa nada....algo pasa con el `Articulo`.`usuarioId`=".$userId." e la linea 178     
